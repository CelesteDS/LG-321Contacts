...
const addContacts = function (contacts) {
    ...
    return errExtant ? (printErrors(errorLog)) : ''
}

// new function for printing errors
const printErrors = function (errArr) {
    var errMsg = `Could not import ${errArr.length} contacts.`
    errArr.forEach(function (entry) {
      errMsg += `\nFirst: ${entry.firstName} , Last: ${entry.lastName}, Email: ${entry.email}`
    })
    return errMsg
}
...
const printContacts = function (errorMsg) {
  ...
  }, 4) // gets the length of the longest name
  const emailCount = 1 + contactStorage.reduce(function (min, currentValue) {
    return Math.max(min, currentValue.email.length)
  }, 5) // gets the length of the longest email
